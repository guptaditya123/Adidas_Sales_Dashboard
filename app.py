...
# Layout with two columns for header
col1, col2 = st.columns([0.1, 0.9])
with col1:
    st.image(image, width=100)

# Title styling and rendering
html_title = """
<style>
    .title_test {
        font-weight: bold;
...
<center><h1 class="title_test">Adidas Interactive Sales Dashboard</h1></center>
"""
with col2:
...
# Retailer-wise data: expander + download
...
# Bar + Line Chart: Sales & Units Sold by State
...
# Expander and download for result1
...
treemap = df[["Region", "City", "TotalSales"]].groupby(["Region", "City"])["TotalSales"].sum().reset_index()

def format_sales(value):
    return '{:.2f} Lakh'.format(value / 100000) if value >= 0 else '0'
...
# Expander and download for region-city sales
